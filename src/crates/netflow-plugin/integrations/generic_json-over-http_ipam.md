...
The catch-all integration for any HTTP endpoint that returns JSON describing your
network prefixes. The plugin does not care WHO produces the JSON -- it cares about
the JSON shape after your `transform` (a jq expression) runs over it. Examples:
Infoblox WAPI, BlueCat REST API, phpIPAM, ServiceNow CMDB queries, an internal
aggregator, a Lambda function, a static file served from S3, a hand-rolled CMDB.
