...
MaxMind is the originator of the MMDB binary format and the canonical commercial
source for geo and ASN data. The netflow plugin reads MaxMind MMDB files directly;
they are interchangeable with other MMDB providers (DB-IP, custom builds).
...
(`apt install geoipupdate`, `dnf install GeoIP-update`) or download from
