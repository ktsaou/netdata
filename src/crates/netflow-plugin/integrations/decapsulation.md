...
<img src="https://netdata.cloud/img/tunnel.svg" width="150"/>
...
Decapsulation rewrites the 5-tuple of a flow record from the **outer** (tunnel)
endpoints to the **inner** (real) endpoints when the exporter ships the encapsulated
frame bytes alongside the standard fields. Without it, every VM-to-VM conversation
on a VXLAN fabric or every inner flow on an SRv6 transit shows up as the same
tunnel pair (hypervisor-to-hypervisor or PE-to-PE), which tells you nothing about
the actual traffic.
...
[Decapsulation](https://learn.netdata.cloud/docs/network-flows/enrichment/decapsulation).
...
must choose -- or run two plugin instances on different ports.
