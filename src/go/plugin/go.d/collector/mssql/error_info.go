...
func (c *Collector) collectErrorInfo(ctx context.Context) *module.FunctionResponse {
	...
	if err != nil {
		if isDeadlockPermissionError(err) {
			return &module.FunctionResponse{Status: 403, Message: deadlockPermissionMessage()}
		}
		if status == mssqlErrorAttrNotEnabled {
			return &module.FunctionResponse{Status: 503, Message: "error-info not enabled: Extended Events session not found or ring_buffer target missing"}
		}
...
func mssqlErrorAttributionColumns() []mssqlColumnMeta {
	return []mssqlColumnMeta{
		{
...
func mssqlPlanAttributionColumns() []mssqlColumnMeta {
	return []mssqlColumnMeta{
		{
			uiKey:       "hashMatch",
			displayName: "Hash Match Joins",
			dataType:    ftInteger,
			visible:     true,
			transform:   trNumber,
			sortDir:     sortDesc,
...
func (c *Collector) collectMSSQLPlanOps(ctx context.Context, data [][]any, cols []mssqlColumnMeta) map[string]map[string]mssqlPlanOps {
	dbIdx := -1
	hashIdx := -1
