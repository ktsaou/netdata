...
static int check_if_label_matches_rrdcalctemplate_pattern_callback(const char *name, const char *value, RRDLABEL_SRC ls, void *data) {
    (void)ls;
...
static int rrdcalctemplate_is_there_label_restriction(RRDCALCTEMPLATE *rt,  RRDHOST *host) {
    ...
    if(host->host_labels) {
        if(rrdlabels_walkthrough_read(
                host->host_labels, check_if_label_matches_rrdcalctemplate_pattern_callback, rt->splabels) != -1) {
