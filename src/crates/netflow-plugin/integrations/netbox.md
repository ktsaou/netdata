...
[NetBox](https://github.com/netbox-community/netbox) is the most widely deployed
open-source IPAM / DCIM. It is Apache-2.0 licensed and maintained by the
netbox-community organization (commercial variants -- NetBox Cloud and NetBox
Enterprise -- are offered by NetBox Labs on top of the same upstream code).
Operators already curate prefix metadata in NetBox -- tenant, site, role, VRF,
description -- and want flow records to inherit those labels automatically rather
than maintaining a parallel list in `netflow.yaml`.
