...
static char *strndup_first_run(const char *s);
static char *(*libc_strndup)(const char *) = strndup_first_run;
...
char *strndupz_int(const char *s, size_t size, const char *file, const char *function, size_t line) {
    ...
    size_t_atomic_count(add, p->strndup_calls, 1);
    size_t_atomic_count(add, p->allocations, 1);
    size_t_atomic_bytes(add, p->bytes, size);
...
char *strndupz(const char *s, size_t size) {
    char *t = strndup(s, size);
    if (unlikely(!t)) fatal("Cannot strndup() string '%s'", s);
    return t;
}
