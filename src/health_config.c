...
static inline int rrdcalc_add_alarm_from_config(RRDHOST *host, RRDCALC *rc) {
    ...
    debug(D_HEALTH, "Health configuration adding alarm '%s.%s' (%u): exec '%s', recipient '%s', green " CALCULATED_NUMBER_FORMAT_AUTO ", red " CALCULATED_NUMBER_FORMAT_AUTO ", lookup: group %d, after %d, before %d, options %u, dimensions '%s', update every %d, calculation '%s', warning '%s', critical '%s', source '%s', delay up %d, delay down %d, delay max %d, delay_multiplier %f, repeat warning %d, repeat critical %d",
            ...
            rc->delay_multiplier,
            rc->repeat_warning_duration,
            rc->repeat_critical_duration
    );
...
static inline int rrdcalctemplate_add_template_from_config(RRDHOST *host, RRDCALCTEMPLATE *rt) {
    ...
    debug(D_HEALTH, "Health configuration adding template '%s': context '%s', exec '%s', recipient '%s', green " CALCULATED_NUMBER_FORMAT_AUTO ", red " CALCULATED_NUMBER_FORMAT_AUTO ", lookup: group %d, after %d, before %d, options %u, dimensions '%s', update every %d, calculation '%s', warning '%s', critical '%s', source '%s', delay up %d, delay down %d, delay max %d, delay_multiplier %f, repeat warning %d, repeat critical %d",
            ...
            rt->delay_multiplier,
            rt->repeat_warning_duration,
            rt->repeat_critical_duration
    );
    ...
    while(*s) {
        ...
        if(!strcasecmp(key, "warn") || !strcasecmp(key, "warning")) {
            ...
            if (!config_parse_duration(key, &default_duration)) {
                error("Health configuration at line %zu of file '%s/%s': invalid value '%s'",
                        line, path, file, key);
            }
    ...
    while((s = fgets(&buffer[append], (int)(HEALTH_CONF_MAX_LINE - append), fp)) || append) {
        ...
        *s = '\0';
        ...
        if(hash == hash_alarm && !strcasecmp(key, HEALTH_ALARM_KEY)) {
            ...
            rc->repeat_warning_duration = host->health_default_repeat_warning_duration;
            rc->repeat_critical_duration = host->health_default_repeat_critical_duration;
            ...
            rt->repeat_warning_duration = host->health_default_repeat_warning_duration;
            rt->repeat_critical_duration = host->health_default_repeat_critical_duration;
            ...
            if(hash == hash_on && !strcasecmp(key, HEALTH_ON_KEY)) {
                ...
                health_parse_repeat(line, path, filename, value, &rc->repeat_warning_duration, &rc->repeat_critical_duration);
            }
            else {
            ...
            if(hash == hash_on && !strcasecmp(key, HEALTH_ON_KEY)) {
                ...
                health_parse_repeat(line, path, filename, value, &rt->repeat_warning_duration, &rt->repeat_critical_duration);
            }
            else {
