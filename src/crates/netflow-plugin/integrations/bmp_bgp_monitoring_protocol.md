...
to a passive collector. With this integration enabled, Netdata is that collector --
it listens for BMP TCP connections from your routers, parses BGP UPDATE messages
carried inside RouteMonitoring frames, and builds an in-memory routing trie that
flow enrichment then reads from.
