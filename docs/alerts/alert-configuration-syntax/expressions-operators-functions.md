...
For hysteresis (different thresholds for entering vs exiting a state), use status-aware expressions:

```conf
# Hysteresis: enter WARNING at 80, exit when below 70
warn: $this > ($status >= $WARNING ? 70 : 80)
crit: $this > 95
```

This creates different thresholds for entering and exiting states. See **8.1 Hysteresis and Status-Based Conditions** for more examples.
