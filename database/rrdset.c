...
//  while(st->alarms)     rrdsetcalc_unlink(st->alarms);
    ...
    labels_destroy(st->state->labels.head);
    freez(st->state);
    freez(st->chart_uuid);
...
RRDSET *rrdset_create_custom(
    ...
    st->state->labels.head = labels_create();
...
void rrdset_update_labels(RRDSET *st, DICTIONARY *labels) {
    if(!st->state->labels.head)
        st->state->labels.head = labels_create();

    if (labels)
        labels_copy_and_replace_existing(st->state->labels.head, labels);

    // TODO - we should also cleanup sqlite from old labels that have been removed
    labels_walkthrough_read(st->state->labels.head, chart_label_store_to_sql_callback, st);
}
...
int rrdset_labels_match_keys_and_values(RRDSET *st, char *keylist, char *words[], int *word_count, int size) {
    if (!st->state->labels.head) return 0;
    ...
    for (int i = 0; ret && i < *word_count - 1; i += 2)
        ret = (labels_match_key_and_value(st->state->labels.head, words[i], words[i+1]))?1:0;
