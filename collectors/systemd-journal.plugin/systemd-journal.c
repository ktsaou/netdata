...
#ifndef SD_JOURNAL_FAST_QUERY
#define SD_JOURNAL_FAST_QUERY (1 << 8)
#endif
...
static ND_SD_JOURNAL_STATUS netdata_systemd_journal_query_one_file(
    ...
    if(sd_journal_open_files_fd(&j, &fd, 1, SD_JOURNAL_FAST_QUERY) < 0 || !j) {
    ...
    if(sd_journal_open_files(&j, paths, SD_JOURNAL_FAST_QUERY) < 0 || !j)
        return ND_SD_JOURNAL_FAILED_TO_OPEN;
...
static void journal_file_update_msg_ut(const char *filename, struct journal_file *jf) {
    ...
    sd_journal *j = NULL;
    if(sd_journal_open_files(&j, files, SD_JOURNAL_FAST_QUERY) < 0 || !j) {
